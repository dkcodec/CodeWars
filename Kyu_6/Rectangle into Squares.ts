/*
    ...
    Notes:
        ...
        in that case,return None/nil/null/Nothing or return {} with C++, 
...
export const sqInRect = (ln: number, wd: number): null | number[] => {
  let arr: number[] = [];
  if (ln === wd) return null;
  while (ln > 0 && wd > 0) {
    arr.push(ln > wd ? wd : ln);
    ln > wd ? (ln -= wd) : (wd -= ln);
  }
  return arr;
};
